...
In order to correctly account for the interactions and decays of these particles, the \textbf{CORSIKA} generator from the KASCADE Collaboration is used. 
...
This tool, known as \textbf{MuonGun}, has the benefit of removing the computationally costly simulation of the full air shower and giving the user more control over the resulting simulated events.

In this generator, the muons are produced on a \textbf{generation cylinder} with a radius of 800 meters and length of 1600 meters, matching the final muon positions of the CORSIKA generator.
...
The angular spectrum of the MuonGun simulation is created by setting a \textbf{target cylinder} toward which the generated muon must intersect.
...
These events, labeled \textbf{noise triggers}, arise due to the low trigger threshold. 
...
Because the events are relatively rare, the simluation requires a special mode, here called \textbf{long-frame} simulation, which produces continuous detector readout. 
...
After triggering, specialized code, known as \textbf{CoincidenceAfterProcessing} is used to divide the long-frame simulation into smaller events more similar to both other simulation as well as actual experimental readout.
...
At energies ranging from approximately 1 GeV to 1 TeV, IceCube has adopted the \textbf{GENIE} event generator.
...
The \textbf{Neutrino-Generator} code (hereafter, \textbf{NuGen}) is designed to handle these higher energy interactions.
